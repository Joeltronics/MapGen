...
def to_image(
		...
		precipitation_mm_per_year: np.ndarray,
		) -> np.ndarray:
	...
	colormap_rainfall_idx = rescale(precipitation_mm_per_year, COLORMAP_PRECIPITATION_RANGE_MM, (0., COLORMAP_RESOLUTION-1.), clip=True)
...
def biome_map(elevation_11: np.ndarray, temperature_C: np.ndarray, precipitation_mm_per_year: np.ndarray):

	if not (elevation_11.shape == temperature_C.shape == precipitation_mm_per_year.shape):
		raise ValueError(f'Arrays do not have the same shape: {elevation_11.shape}, {temperature_C.shape}, {precipitation_mm_per_year.shape}')

	temperature_01 = rescale(temperature_C, BIOME_MAP_TEMPERATURE_RANGE_C, (0., 1.), clip=True)
	rainfall_01 = rescale(precipitation_mm_per_year, BIOME_MAP_PRECIPITATION_RANGE_MM, (0., 1.0), clip=True)
